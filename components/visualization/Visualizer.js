...
class Visualizer {
      ...
      render: function() {
        ...
        if (debugPane)
          debugPane.textContent =
            "Debug info: " +
            "\nzoom = " +
            self.map.getZoom().toFixed(2) +
            ";\ncentre = " +
            self.map.getCenter().toString();
  ...
  startAnimation(allData, totalDesiredRuntimeMs = 3 * 60 * 1000 /*3 min*/) {
    ...
    const refreshIntervalMs = Math.ceil(
      totalDesiredRuntimeMs / totalNumberSteps
    );
    ...
    var timer = window.setInterval(() => {
      ...
      for (let dataSetIdx = 0; dataSetIdx < allData.length; ++dataSetIdx) {
        ...
        if (typeof displayedEventIndices[dataSetIdx] === "undefined")
          displayedEventIndices.push(new Array());
        const currentEventIndices = displayedEventIndices[dataSetIdx];

        const dataset = allData[dataSetIdx].data;
        this.store.set(
          `displayedEvent${dataSetIdx}`,
          currentEventIndices.map(index => {
        ...
        if (this.removeOldEvents == true) {
          for (let i = currentEventIndices.length - 1; i >= 0; --i) {
            ...
            if (
              this._strToMs(dataset[eventIdx].dateStart) +
                dataset[eventIdx].duration <=
              virtualTime
            ) {
      ...
      for (let dataSetIdx = 0; dataSetIdx < allData.length; ++dataSetIdx) {
        if (typeof eventIndices[dataSetIdx] === "undefined")
          eventIndices.push(0);
        ...
        if (i < dataset.length) {
          ...
          for (
            ;
            i < dataset.length &&
            this._strToMs(dataset[i].dateStart) <= virtualTime;
            ++i
          ) {
            ...
            if (
              dataset[i].description !== undefined &&
      ...
      this.store.set(
        "progress",
        `${((virtualTime - earliestDateMs) / dateRangeMs) * 100}%`
      );
